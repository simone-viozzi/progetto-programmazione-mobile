...
class ArchiveAdapter(
...
) : ListAdapter<ArchiveDataModel, ArchiveAdapter.ArchiveViewHolder>(ArchiveDiffCallback()){
    ...
    ) : RecyclerView.ViewHolder(binding.root)
    {
        class AggregateViewHolder(
            ...
            private var defaultBitmap: Boolean = false
                ...
                with(binding){
                    ...
                    if(aggregate.thumbnail != null){
                        ...
                        attachemntRepository.generateThumbnailFromUri(aggregate.thumbnail)?.let{
                            ...
                            //Log.d("archive_rv","imageView src updated with coustom bitmap")
                        }
                    }else{
                        if(!defaultBitmap){
                            defaultBitmap = true
                            imageAttachment.setImageBitmap(attachemntRepository.getDefaultBitmap())
                            //Log.d("archive_rv", "imageView src updated with default bitmap")
                        }else{}
                    }

                    aggregateCard.setOnClickListener{
                        navController.navigate(
                            ArchiveFragmentDirections
                                .actionArchiveFragmentToAggregateFragment(
                                aggregate.aggr_id
                            )
                        )
                    }
